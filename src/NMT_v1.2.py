...
def create_model(de_vocab_size, en_vocab_size, max_len):
    model = Sequential()
    model.add(Embedding(input_dim=de_vocab_size, output_dim=512, input_length=max_len))
    model.add(Bidirectional(LSTM(512, return_sequences=True, dropout=0.5, recurrent_dropout=0.5)))
    model.add(Dense(en_vocab_size, activation='softmax', kernel_regularizer=regularizers.l2(0.01)))
...
def train_model(model, de_train_pad, target_data):
    model.fit(de_train_pad, target_data, batch_size=64, epochs=10, validation_split=0.2)
